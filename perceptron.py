#-------------------------------------------------------------------------
# AUTHOR: Andy Lam
# FILENAME: CS4210Assignment4
# SPECIFICATION: Simulate a grid search, trying to find which combination of two hyperparameters (learning rate and shuffle) leads you to the best prediction performance for each classifier.
...
for w in n: #iterates over n
    ...
    for b in r: #iterates over r
        ...
        for a in range(2): #iterates over the algorithms
            ...
            if a==0 and ca > ha:
                ha = ca
                print("Highest Perceptron accuracy so far: %f, Parameters: learning rate=%f, shuffle=%s" %(ha, w, b))
            elif a!=0 and ca > ha1:
                ha1 = ca
                print("Highest MLP accuracy so far: %f, Parameters: learn rate=%f, shuffle=%s" %(ha1, w, b))
