...
# SPECIFICATION: Train and test multiple deep neural networks and check their corresponding performances, always updating the highest accuracy found.
...
for h in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for n in n_neurons:                     #looking or the best parameters w.r.t the number of neurons
        for l in l_rate:                    #looking or the best parameters w.r.t the learning rate
            ...
            i = 0
            for y in y_test:
                if class_predicted[i] == y:
                    ac += 1
                i+=1
            ca = ac/len(X_test)
            if ca > highestAccuracy:
                highestAccuracy = ca
